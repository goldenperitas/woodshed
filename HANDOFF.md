...
1. **ストレージ退避への備え**（未着手）。iOS は使われない期間が続くと OPFS /
   IndexedDB を捨てる。捨てられるとメタデータはMacから戻せるが、**音源は
   端末にしか無いので戻せない**。方針はユーザー確認済みで「音源のバックアップ
   手段も要る」。ただし **Macに音源を置く形にするなら §5 の確認が要る**
   （現在の整理は「サーバーはメタデータのみ」で成り立っている）
2. 退避の検知と、メタデータの自動再同期
